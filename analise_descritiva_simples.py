...
print("(Incluindo deputados distritais do DF - Excluindo localidades no exterior)")
print("="*80)

# Caminhos base
DATA_DIR = 'C:/Users/João Pedro/Downloads/more-lula-more-women--/data'
...
arquivo_pres = os.path.join(DATA_DIR, 'votacao_secao_2022_BR_REDUCED.csv')
...
print(f" Municípios/localidades processados: {len(pres_pivot)}")

# ==============================================================================
# PARTE 2: IDENTIFICAR GÊNERO - DEPUTADOS FEDERAIS E ESTADUAIS (+ DISTRITAIS)
# ==============================================================================

print("\n[2/5] Identificando gênero dos candidatos...")

arquivo_cand = os.path.join(CAND_DIR, 'consulta_cand_2022_BRASIL_REDUCED.csv')

print(f"\n Lendo arquivo: {arquivo_cand}")

# Verificar estrutura primeiro
amostra = pd.read_csv(arquivo_cand, sep=';', encoding='utf-8-sig', nrows=10)
...
# Filtrar deputados (federal, estadual E DISTRITAL), 1º turno
# CORREÇÃO: Incluir CDCARGO 8 (deputados distritais do DF)
cands_dep = cands[(cands['NR_TURNO'] == 1) & (cands['CD_CARGO'].isin([6, 7, 8]))].copy()

print(f"\n Candidatos a deputado (federal + estadual + distrital) no 1º turno: {len(cands_dep)}")

# Separar por cargo
cands_fed = cands_dep[cands_dep['CD_CARGO'] == 6].copy()
cands_est = cands_dep[cands_dep['CD_CARGO'].isin([7, 8])].copy()  # CORREÇÃO: Incluir CDCARGO 8
...
print(f"\n DEPUTADO ESTADUAL + DISTRITAL (cargo 7 + 8):")
...
if len(cands_est) > 0:
    ...
    print(" AVISO: Nenhum candidato estadual/distrital encontrado!")
    genero_map_est = {}
...
arquivo_suf = os.path.join(DATA_DIR, 'votacao_secao_2022_{}_REDUCED.csv')

votos_dep_fed_list = []

for uf in states:
    arq = arquivo_suf.format(uf)
    
    print(f" Processando {uf}...", end='', flush=True)
    
    chunksize = 300000 if uf in ['SP', 'MG', 'BA', 'MA', 'RJ', 'RS', 'PR'] else 500000
    ...
    chunks = pd.read_csv(
        ...
        encoding='utf-8-sig',
        chunksize=chunksize,
        usecols=['NR_TURNO', 'CD_CARGO', 'SG_UF', 'CD_MUNICIPIO', 'NR_VOTAVEL', 'QT_VOTOS'],
        dtype={'NR_TURNO': 'int8', 'CD_CARGO': 'int8', 'NR_VOTAVEL': 'int32', 'QT_VOTOS': 'int32', 'CD_MUNICIPIO': 'int32'}
    )
    ...
    for chunk in chunks:
        ...
        agg = chunk.groupby(['SG_UF', 'CD_MUNICIPIO', 'NR_VOTAVEL'], as_index=False)['QT_VOTOS'].sum()
        uf_agg_list.append(agg)
    
    if uf_agg_list:
        uf_agg = pd.concat(uf_agg_list, ignore_index=True)
        uf_agg = uf_agg.groupby(['SG_UF', 'CD_MUNICIPIO', 'NR_VOTAVEL'], as_index=False)['QT_VOTOS'].sum()
...
if votos_dep_fed_list:
    ...
    gc.collect()
    print(f" Total de registros: {len(votos_dep_fed)}")
    
    votos_dep_fed['eh_mulher'] = votos_dep_fed.apply(
        lambda r: genero_map_fed.get((r['SG_UF'], r['NR_VOTAVEL']), 0), axis=1
    )
else:
    print(" AVISO: Nenhum voto em deputado federal encontrado!")
    votos_dep_fed = pd.DataFrame(columns=['SG_UF', 'CD_MUNICIPIO', 'NR_VOTAVEL', 'QT_VOTOS', 'eh_mulher'])

# ==============================================================================
# PARTE 4: PROCESSAR VOTAÇÃO EM DEPUTADOS ESTADUAIS E DISTRITAIS
# ==============================================================================

print("\n[4/5] Processando votação em DEPUTADOS ESTADUAIS e DISTRITAIS por UF...")
...
for uf in states:
    ...
    for chunk in chunks:
        # CORREÇÃO: Incluir CDCARGO 7 (estaduais) E CDCARGO 8 (distritais do DF)
        chunk = chunk[(chunk['NR_TURNO'] == 1) & (chunk['CD_CARGO'].isin([7, 8]))]
        agg = chunk.groupby(['SG_UF', 'CD_MUNICIPIO', 'NR_VOTAVEL'], as_index=False)['QT_VOTOS'].sum()
        uf_agg_list.append(agg)
...
if votos_dep_est_list:
    ...
    gc.collect()
    print(f" Total de registros: {len(votos_dep_est)}")
    
    votos_dep_est['eh_mulher'] = votos_dep_est.apply(
        lambda r: genero_map_est.get((r['SG_UF'], r['NR_VOTAVEL']), 0), axis=1
    )
else:
    print(" AVISO: Nenhum voto em deputado estadual/distrital encontrado!")
    votos_dep_est = pd.DataFrame(columns=['SG_UF', 'CD_MUNICIPIO', 'NR_VOTAVEL', 'QT_VOTOS', 'eh_mulher'])

# ==============================================================================
# PARTE 5: AGREGAR DADOS E CRIAR TABELAS FINAIS
# ==============================================================================

print("\n[5/5] Agregando dados e criando tabelas finais...")

if len(votos_dep_fed) > 0:
    votos_fed_mun = votos_dep_fed.groupby(['SG_UF', 'CD_MUNICIPIO'], as_index=False).agg({'QT_VOTOS': 'sum'})
    ...
    votos_fed_mulheres = votos_dep_fed[votos_dep_fed['eh_mulher'] == 1].groupby(
        ['SG_UF', 'CD_MUNICIPIO'], as_index=False)['QT_VOTOS'].sum()
...
if len(votos_dep_est) > 0:
    votos_est_mun = votos_dep_est.groupby(['SG_UF', 'CD_MUNICIPIO'], as_index=False).agg({'QT_VOTOS': 'sum'})
...
# 🎯 FILTRO IMPORTANTE: Remover localidades fora do Brasil (sem votos em deputados)
df_final = df_final[df_final['total_votos_deputados'] > 0].copy()

print(f"\n Após filtrar exterior: {len(df_final)} municípios brasileiros")
...
print(" - num_eleitores: Total de votos válidos em deputados (federal + estadual + distrital)")
print(" - perc_lula: % de votos em Lula no 2º turno presidencial")
print(" - perc_votos_mulheres_fed: % de votos em deputadas FEDERAIS")
print(" - perc_votos_mulheres_est: % de votos em deputadas ESTADUAIS + DISTRITAIS")
print(" - perc_votos_mulheres_total: % de votos em deputadas (fed + est + dist)")

print("\nMETODOLOGIA:")
print(" - Dados limitados aos municípios brasileiros com votação completa")
print(" - Excluídas localidades no exterior (Buenos Aires, Paris, Nova York, etc.)")
print(" - Incluídos deputados distritais do DF (CDCARGO 8)")
